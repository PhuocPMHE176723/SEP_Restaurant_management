...
namespace rmn_be.Core.Services.Implementation
{
    public class StaffService : IStaffService
    {
        ...
        public async Task<PagedResultDTO<StaffDTO>> GetAllStaffAsync(PagingRequestDTO pagingRequest)
        {
            ...
            if (!string.IsNullOrWhiteSpace(pagingRequest.SearchTerm))
            {
                ...
                query = query.Where(x =>
                    ...
                    (!string.IsNullOrEmpty(x.StaffCode) && x.StaffCode.ToLower().Contains(keyword))
                );
            }

            if (!string.IsNullOrWhiteSpace(pagingRequest.Status) &&
                pagingRequest.Status.ToUpper() != "ALL")
            {
                query = query.Where(x => x.WorkingStatus.ToString().ToUpper() == pagingRequest.Status.ToUpper());
            }

            query = query
                .OrderBy(x => x.WorkingStatus.Equals("ACTIVE"))
                .ThenByDescending(x => x.CreatedAt);

            var totalRecords = query.Count();

            var pagedStaffs = query
        ...
        ";
                ...
                await _emailService.SendEmailNewAsync(email, subject, body);
            }
            return _mapper.Map<StaffDTO>(staff);
        }
        ...
        {
            Console.WriteLine($"GetMyProfileAsync userId: {userId}");

            var staffs = await _unitOfWork
                .GetRepository<Staff>()
                .GetAllAsync();

            Console.WriteLine($"Total staffs: {staffs.Count()}");

            var entity = staffs.FirstOrDefault(s => s.UserId == userId);

            Console.WriteLine($"Found entity: {entity != null}");

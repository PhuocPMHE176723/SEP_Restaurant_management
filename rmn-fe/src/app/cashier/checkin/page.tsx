...
export default function CashierCheckinPage() {
  ...
  const handleCheckin = async (reservationId: number, tableId: number) => {
    ...
    if (res && !isCheckinTimeValid(res.reservedAt)) {
      showError(
        "Lỗi",
        "Chỉ có thể check-in trong khoảng 30 phút so với giờ đặt bàn.",
      );
      return;
    }
    ...
    try {
      ...
      if (result?.orderId) {
        window.location.href = `/cashier/checkout/${result.orderId}`;
      }
    } catch (error) {
  ...
  const selectedReservation = filteredReservations.find(
    (r) => r.reservationId === selectedReservationId,
  );
                    ...
                    {currentReservations.map((r) => (
                      <tr
                        ...
                        style={{
                          cursor: "pointer",
                          backgroundColor:
                            selectedReservationId === r.reservationId
                              ? "#eff6ff"
                              : "transparent",
                        }}
                      >
                        <td style={{ fontWeight: 700 }}>
                          {new Date(r.reservedAt).toLocaleTimeString("vi-VN", {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                        </td>
                  ...
                  <div>
                    ...
                    >
                      {suitableTables.length === 0 ? (
                        ...
                        >
                          Không có bàn trống đủ sức chứa (

...
export default function KitchenPage() {
  ...
  const [currentItem, setCurrentItem] = useState<OrderItemResponse | null>(
    null,
  );
  ...
  const toggleIngredient = (id: number) => {
    if (selectedIngredients.find((i) => i.ingredientId === id)) {
      setSelectedIngredients(
        selectedIngredients.filter((i) => i.ingredientId !== id),
      );
    } else {
      setSelectedIngredients([
        ...selectedIngredients,
        { ingredientId: id, quantity: 1 },
      ]);
    }
  };
  ...
  return (
    <div className={styles.pageContainer}>
      <div style={{ padding: "0 1.5rem", marginTop: "1rem" }}>
        <h1 className={styles.tableName} style={{ fontSize: "2rem" }}>
          Bảng điều hành nhà bếp
        </h1>
        <p style={{ color: "#64748b" }}>
          Quản lý danh sách các món ăn đang được yêu cầu chế biến
        </p>
      </div>
      ...
      <div className={styles.kitchenGrid} style={{ marginTop: "2rem" }}>
        {activeOrders.length === 0 ? (
          <div
            style={{ textAlign: "center", gridColumn: "1/-1", padding: "5rem" }}
          >
            <div style={{ fontSize: "3rem", marginBottom: "1rem" }}>👨‍🍳</div>
            <h3>Hiện không có yêu cầu nào</h3>
            <p>
              Khi có yêu cầu mới từ nhân viên hoặc khách hàng, chúng sẽ xuất
              hiện ở đây.
            </p>
          </div>
        ) : (
          activeOrders.map((order) => (
            <div key={order.orderId} className={styles.orderCard}>
              <div className={styles.cardHeader}>
                ...
                <span
                  className={`${styles.statusBadge} ${order.status === "OPEN" ? styles.statusOpen : styles.statusKitchen}`}
                >
                  ...
                  {order.orderItems.map((item) => {
                    ...
                    return (
                      <li
                        ...
                        className={`${styles.itemRow} ${status === "COOKING" ? styles.itemRowCooking : styles.itemRowPending}`}
                      >
              ...
              <div className={styles.cardFooter}>
                <button
                  ...
                  onClick={() =>
                    handleUpdateOrderStatus(order.orderId, "SERVED")
                  }
                >
                  Hoàn tất cả bàn
                </button>
          ...
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              <h2 className={styles.modalTitle}>
                Điều chỉnh nguyên liệu tiêu hao
              </h2>
              <button
                style={{
                  border: "none",
            ...
            <div className={styles.modalBody}>
              ...
              >
                Sử dụng để ghi nhận các nguyên liệu dùng thêm hoặc thay thế cho
                món ăn này.
              </p>
              <p style={{ marginBottom: "1rem", fontWeight: 700 }}>
                Món:{" "}
                <span style={{ color: "#f97316" }}>
                  {currentItem?.itemNameSnapshot}
                </span>
              </p>
              ...
              <div style={{ maxHeight: "300px", overflowY: "auto" }}>
                {ingredients.map((ing) => {
                  const selected = selectedIngredients.find(
                    (si) => si.ingredientId === ing.ingredientId,
                  );
                  return (
                    <div

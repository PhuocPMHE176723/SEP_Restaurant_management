...
export default function TableTransferPage() {
      ...
      const tablesWithOrders: TableWithOrder[] = (tablesData || []).map(
        (table) => {
          const currentOrder = Array.isArray(ordersData)
            ? ordersData.find(
                (order) =>
                  (order.tableIds?.includes(table.tableId) ||
  ...
  const canTransferTo = (table: TableWithOrder) => {
    return (
      table.status === "AVAILABLE" && !table.currentOrder && table.isActive
    );
  };
  ...
  const handleTransfer = async () => {
    if (!selectedFromTable) {
      showError(
        "Chưa chọn bàn nguồn",
        "Vui lòng chọn một bàn đang có khách để chuyển.",
      );
      return;
    }
    if (!selectedToTable) {
      showError(
        "Chưa chọn bàn đích",
        "Vui lòng chọn một bàn trống để chuyển đến.",
      );
      return;
    }
    if (!transferReason.trim()) {
      showWarning(
        "Chưa nhập lý do",
        "Vui lòng nhập lý do chuyển bàn trước khi xác nhận.",
      );
      return;
    }
    ...
    try {
      ...
      showSuccess(
        "Thành công",
        `Đã chuyển khách từ bàn ${selectedFromTable.tableName || selectedFromTable.tableCode} sang ${selectedToTable.tableName || selectedToTable.tableCode}`,
      );
      ...
      showError(
        "Lỗi chuyển bàn",
        error.message || "Có lỗi xảy ra khi chuyển bàn.",
      );
    }
  };
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        className={`${styles.badge} ${styles.badgeCompact} ${getStatusClass(table.status)}`}
                      >
                  ...
                  {selectedToTable ? (
                    <div>
                      ...
                      <div style={{ fontSize: "0.85rem", color: "#10b981" }}>
                        {selectedToTable.capacity} chỗ ngồi
                      </div>
                    </div>
                  ) : (
                    <div style={{ fontSize: "0.85rem", color: "#94a3b8" }}>
                      Chạm vào bàn trống mới
                    </div>
                  )}
                </div>
